...
#define WATERAVAIABLE_ACE_TOPIC "tele/hydro_box1/WATERAVAIABLE_ACE" //Checkt ob Wasser fließt beim NFT System ACE
#define WATERAVAIABLE_BD_TOPIC "tele/hydro_box1/WATERAVAIABLE_BD" //Checkt ob Wasser fließt beim NFT System BD
...
void mqttconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      if((mqtt_zeit-millis()-4294967216.0) < (-60000)){ESP.restart();}
      delay(5000);
    }
  }
}
...
void loop()
{
  ...
  Serial.print("WaterStatus ACE: "); Serial.println(waterStatus_ACE);
  Serial.print("WaterStatus BD: "); Serial.println(waterStatus_BD);
  delay(1000);
//BH1750
...
*/
delay(1000);
  //DS18B20, Auflösung "nur" 10Bit, da sonst Timing Problem mit WiFi... Mit 10 Bit gelingt jede 2te Messung...
...
for(int j=0; j < dallas_count+2;j++){
  switch(j){
  case 0: break;
  case 1: dallas_AT=celsius;Serial.println(dallas_AT);break;
  case 2: dallas_WT=celsius;Serial.println(dallas_WT);break;
  case 3: dallas_TT=celsius;Serial.println(dallas_TT);break;
  default: delay(2000);
//Verbinde mit WLAN
  int wlan_zeit = millis();
  WiFi.begin(ssid, password);

  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
  if((wlan_zeit-millis()-4294967216.0) < (-60000)){ESP.restart();}
  Serial.println((wlan_zeit-millis()-4294967216.0));
    Serial.print(".");
  }
